	...
	public LeafNode[] divideLeafNode() {
		...
		for (int i = 0; i < clusterChilds.size() - 1; i++) {
			...
			for (int j = i + 1; j < clusterChilds.size(); j++) {
				temp = ClusteringFeature.computerClusterDistance(tempCluster, clusterChilds.get(j));
		...
		for (Cluster c : clusterChilds) {
			if (ClusteringFeature.computerClusterDistance(c, cluster1) < ClusteringFeature.computerClusterDistance(c, cluster2)) {
